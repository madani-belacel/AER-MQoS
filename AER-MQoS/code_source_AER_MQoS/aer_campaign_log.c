 ...
 *
 * Lignes METRIC,... que parse_cooja_logs.py utilise.
 * Desactivable avec AER_CONF_CAMPAIGN_METRICS=0.
 * Chaque ligne commence par "METRIC," sur la console Cooja.
 *
 * METRIC log lines for the Python CSV pipeline.
 */
...
time_ms(void)
{
  return (uint32_t)((1000ULL * (unsigned long)clock_time()) /
