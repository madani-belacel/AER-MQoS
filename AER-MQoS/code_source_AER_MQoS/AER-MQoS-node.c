 ...
 *
 * Noeud de campagne Cooja : trafic UDP, files WRR, logging METRIC.
 * Point d'entree du firmware AER-MQoS pour les simulations.
 * Chaque noeud emet selon un profil heterogene (node_id % 4).
 *
 * Cooja node entry point: traffic generation, WRR, METRIC export.
 */
...
#ifndef AER_DRAIN_BUDGET
#define AER_DRAIN_BUDGET 8
#endif
...
#define SEND_INTERVAL_BASE (CLOCK_SECOND / 8)
...
/* On utilise node_id % 4 pour distribuer quatre profils de trafic (C0..C3)
   et obtenir un trafic heterogene dans le reseau (Section VII-B). */
static unsigned
...
PROCESS_THREAD(aer_rpl_plus_node_process, ev, data)
{
  ...
  while(1) {
    ...
    if(etimer_expired(&qos_drain)) {
      ...
      for(drain_budget = 0; drain_budget < AER_DRAIN_BUDGET; drain_budget++) {
        qp = aer_qos_dequeue();
        if(qp == NULL) {
          break;
        }
        ...
        if(simple_udp_sendto(qp->conn, qp->data, qp->len, &qp->dest) == 0) {
          ql_tx_ok_win++;
        }
        ql_tx_tot_win++;
        aer_campaign_log_tx(qp->seq, (uint8_t)qp->traffic_class, qp->len);
