...
# RPL_AER, AER-MQoS), aggregates CSVs (seed / channel / success_ratio),
...
#
# Manuscript bundle (default: four seeds, 1800 s, lossless + lossy sr=0.85):
#   ./run_multi_seed_campaign.sh
#   nohup ./run_multi_seed_campaign.sh > logs/multi_seed/campaign_$(date +%Y%m%d_%H%M).log 2>&1 &
#
# Environment variables:
#   NUM_SEEDS=4           seeds for submission (20260601..20260604)
#   SEED_BASE=20260601    first seed; following = BASE+1 … BASE+NUM_SEEDS-1
#   SEEDS="20260601 20260603"   explicit list (overrides NUM_SEEDS/SEED_BASE)
...
ANALYZE_PY="$PROJECT_ROOT/Section-tex/scripts/analyze_multi_seed_stats.py"
export CONTIKI="${CONTIKI:-$(cd "$PROJECT_ROOT/../.." && pwd)}"
...
SIMDIR="${SIMDIR:-$PROJECT_ROOT/Section-tex/sim/multi_seed}"
...
# Include the paper's archived seed if absent from the auto-generated list
ARCHIVED_SEED=20260601
found_archived=0
...
PY="$PROJECT_ROOT/Section-tex/scripts/parse_cooja_logs.py"
...
echo "  python3 $PROJECT_ROOT/Section-tex/scripts/generate_figures_matplotlib.py --from-csv $SIMDIR"
